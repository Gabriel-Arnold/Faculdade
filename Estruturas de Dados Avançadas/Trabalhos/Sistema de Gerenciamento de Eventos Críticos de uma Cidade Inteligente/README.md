...
- A árvore é ordenada exclusivamente pelo ID do evento.
- A severidade e o status podem ser atualizados sem quebrar a consistência da AVL, pois não alteram a chave de ordenação.
- A remoção é bloqueada para eventos ativos, conforme solicitado no enunciado.
- Não são usadas bibliotecas prontas de árvores.
- Não são usados vetores ou listas para substituir a árvore. O vetor presente na função de exemplos serve apenas para carga de dados de teste, não para armazenamento principal do sistema.
